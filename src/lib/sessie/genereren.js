...
function bouwArchetypeHint(effectiefSessietype, archetypes, recenteArchetypes) {
  ...
  const sessieVariatieBlok = JSON.stringify({
    ...
    rotatie_instructie: isGemengd
      ? "Dit is een vrijheidsessie (gemengd). Kies een archetype dat NIET gelijk is aan recente_archetypes[0]. Geef gekozen_archetype_id terug als apart veld in je JSON-output."
