...
export async function DELETE() {
  try {
    ...
    if (!user) return NextResponse.json({ success: false, error: "Niet ingelogd" }, { status: 401 });
    const kv = getKV();
    await kv.del(`user:${user.id}:intervals_key`);
    await kv.del(`user:${user.id}:athlete_id`);
    await kv.del(`user:${user.id}:athlete_naam`);
    await kv.del(`user:${user.id}:apparaten`);
    await kv.del(`user:${user.id}:onboarding`);
